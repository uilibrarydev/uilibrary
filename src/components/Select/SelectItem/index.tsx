...
export const SelectItem = (props: TSelectItemProps): JSX.Element => {
    ...
    return (
        <div className={`select__option ${disabled ? 'select__option--disabled' : ''}`}>
            {isSelected ? <Icon name="mark" size="xsmall" type={`${disabled ? 'disabled' : 'brand'}`} className="mr-8"/> : null}
            ...
            <div className="select__option__inner" onClick={() => onClick(data.value)}>
                ...
                {leftIconProps ? <Icon {...leftIconProps} size="xsmall" type={`${disabled ? 'disabled' : 'primary'}`} className="mr-8"/> : null}
                <span className="select__option__text">
                  {label}
                </span>
            </div>
            {rightIconProps ? <Icon {...rightIconProps} size="xsmall" type={`${disabled ? 'disabled' : 'primary'}`}  className="ml-8"/> : null}
        </div>
    )
}
